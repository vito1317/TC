...
void free_file(File* file) {
    // Note: Since File and its members are allocated via the custom arena
    // (alloc_memory and create_string), we don't actually call free() here.
    // The memory will be reclaimed when the entire arena is cleaned up or process exits.
    // This function exists to satisfy cleanup semantics expected by callers.
    if (file == NULL) return;
    file->path = NULL;
    file->name = NULL;
    file->extension = NULL;
